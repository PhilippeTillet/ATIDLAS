...
void generator::visit_load_inst(ir::load_inst* x){
  ...
  for(size_t i = 0; i < idxs.size(); i += vec){
    ...
    if(mx){
      ...
      llvm::SplitBlockAndInsertIfThenElse(builder_->getTrue(), _ret, &then_term, &else_term);
...
//  num_ptr_b = 4;
  ...
  std::cout << shared_pre_ptr_[layout_a] << " " << shared_next_ptr_[layout_a] << std::endl;
  ...
  auto register_lds = [&](typeof(has)& vals, int m, int K, int inc, Value* val0, Value* val1){
      if(K == 0){
  ...
  auto load_a = [&](int m, int K, int inc){
      ...
      register_lds(has, m, K, inc, ha00, ha01);
      if(vec_a > 4){
        Value *ha10 = bit_cast(extract_elt(ha, i32(2)), f16x2_ty);
        Value *ha11 = bit_cast(extract_elt(ha, i32(3)), f16x2_ty);
        if(is_a_row)
          register_lds(has, m, K+4, inc, ha10, ha11);
        else
          register_lds(has, m+1, K, inc, ha10, ha11);
      }
  };
  ...
  auto load_b = [&](int n, int K, int inc){
      ...
      register_lds(hbs, n, K, inc, hb00, hb01);
      if(vec_b > 4){
        Value *hb10 = bit_cast(extract_elt(hb, i32(2)), f16x2_ty);
        Value *hb11 = bit_cast(extract_elt(hb, i32(3)), f16x2_ty);
        if(is_b_row)
          register_lds(hbs, n+1, K, inc, hb10, hb11);
        else
          register_lds(hbs, n, K+4, inc, hb10, hb11);
      }
  };
  ...
  for(unsigned m = 0; m < num_m/2; m+=is_a_row?1:2){
    has[{m, 0}].first = builder_->CreatePHI(f16x2_ty, 2);
    has[{m, 0}].second = builder_->CreatePHI(f16x2_ty, 2);
    if(!is_a_row){
      has[{m+1, 0}].first = builder_->CreatePHI(f16x2_ty, 2);
      has[{m+1, 0}].second = builder_->CreatePHI(f16x2_ty, 2);
    }
  }
  for(unsigned n = 0; n < num_n/2; n+=is_b_row?2:1){
    hbs[{n, 0}].first = builder_->CreatePHI(f16x2_ty, 2);
    hbs[{n, 0}].second = builder_->CreatePHI(f16x2_ty, 2);
    if(is_b_row){
      hbs[{n+1, 0}].first = builder_->CreatePHI(f16x2_ty, 2);
      hbs[{n+1, 0}].second = builder_->CreatePHI(f16x2_ty, 2);
    }
  }
